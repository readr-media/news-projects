<template>
  <div id="app">
    ...
    <div class="video-container">
      <iframe class="youtube-video" :src="liveUrl"></iframe>
    </div>

    <div class="section-container">
      <div class="title">辯論即時事實查核成果</div>
      <div class="factcheck-list">
        <Item :key="index" :item="item" v-for="(item, index) in limitedItems" />
        <div class="paging">
          <div class="position">顯示 {{limitNumber}} / {{debates.length}} 則</div>
          <div class="loadmore" @click="loadMore">載入更多</div>
        </div>
      </div>
    </div>
    ...
    <div class="highlight-background">
      <div class="section-container">
        <div class="title">此次參與查核媒體</div>
        <div class="media">
          <a class="block" v-for="(media, index) in mediaList" :key="index" :href="media.url">
            <div class="logo" :class="[{[media.id]: true}]"></div>
          </a>
        </div>
      </div>
    </div>
    ...
    <div class="section-container">
      <div class="share">
        <button class="facebook" @click="shareToFacebook">分享到 Facebook</button>
        <button class="line" @click="shareToLine">分享到 LINE</button>
      </div>
      <a class="action" href="#">看 2020 總統候選人事實查核計畫</a>
    </div>

    <div class="footer">
      <div class="credit">工程師：小島 / 設計：weiweihsu</div>
    </div>
  </div>
</template>
