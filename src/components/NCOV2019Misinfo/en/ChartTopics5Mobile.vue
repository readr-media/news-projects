...
<script>
  ...
  export default {
    ...
    async mounted() {
      ...
      const data = this._dataCSV.filter(d => ['因為疫情衍生出的行為'].includes(d.topics))
      ...
      // wrapper.append('g').call(yAxis)
       ...
       this._y = d3.scaleBand().domain(['因為疫情衍生出的行為']).range([0, noSplitHeight])
       const y = this._y
      ...
      wrapper
        ...
        .attr('x', innerWidth / 2)
        .attr('y', d => y('因為疫情衍生出的行為') + y.bandwidth() / 2 - 70)
