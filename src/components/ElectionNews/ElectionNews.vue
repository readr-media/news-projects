...
import { PROJECTS_BELONG_MM, MM_SITE_NAME, READR_SITE_NAME, MM_SITE_URL, READR_SITE_URL, MM_SITE_ASSETS_URL, READR_SITE_ASSETS_URL, } from 'src/constants'
...
export default {
  metaInfo () {
    ...
    switch (this.mainViewStatus) {
      ...
      default: {
        const title = isEN ? 'Is there specific political inclination on media toward elections? Here is Google artificial intelligence’s answer' : '選舉新聞風向球：媒體報導看風向？Google 人工智慧來解答'
        const description = isEN ? 'Does media have different sentiment when reported on different political figures? For the same candidate, the media reported sentiment before and after the elections will be changed? We use Google artificial intelligence to try to answer these questions and will continue to track until 2020.' : '媒體在報導不同的政治人物時會有情緒差異嗎？針對同一位候選人，媒體報導的情緒在選舉前後會不會有所轉變？我們借助 Google 人工智慧來嘗試解答這些問題，並將持續追蹤至 2020 年。'
