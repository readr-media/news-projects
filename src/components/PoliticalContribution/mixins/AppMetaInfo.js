...
const titleHeadings = '數讀政治獻金 3.0'
...
export default {
  computed: {
    ...
    isQueryOrdinalNumValid () {
      return ['7', '8', '9', '10'].includes(this.queryOrdinalNum)
    },
    queryName () {
  ...
  metaInfo () {
    ...
    switch (this.slug) {
      case 'explore':
        if (this.showLightbox && this.isQueryOrdinalNumValid && this.isQueryNameValid) {
          return {
            ...
            description: `立委${this.queryName}的在 2012 年至 2020 年中 3 次選舉的政治獻金公開資料中，收到哪些集團的捐贈？又集中在哪些產業？快來看看 READr 為你整理的資料。`,
            metaUrl: `political-contribution/explore?name=${this.queryName}&ordinal=${this.queryOrdinalNum}`,
            metaImage: 'political-contribution/ogimgs/ogimage_v2.jpg',
            customScript: structuredDataScript
          }
        } else {
          return {
            title: `${titleHeadings}：立委的金主投資名冊`,
            description: '在 2012 年至 2020 年中 3 次立委選舉的政治獻金公開資料中，立委們分別收到哪些集團的捐贈？又集中在哪些產業？選區內與選區外的捐獻有差異嗎？來看看 READr 為你整理的資料。',
      ...
      case 'dashboard':
        return {
          title: `${titleHeadings}：12 年立委政商關係視覺化一次看！`,
          description: '2008 年至 2020 年 4 次立委選舉中，哪個集團捐最多錢？捐藍還捐綠？政黨傾向是否有變化？哪些產業最喜歡捐贈政治獻金？我們將問題的答案透過視覺化通通放在圖表區，快來找答案！',
      ...
      default: {
        if (isEN) {
          return {
            title: `Political Donations in the Statistics 3.0: Legislator's twelve-year investor list`,
            description: '誰一直是立委的大金主？原來企業捐獻的政治獻金也會藍綠兩邊押寶？READr 藉由鄉民的幫助擴大了立委政治獻金資料庫，來看看 2008 年至 2020 年 4 次立委選舉中候選人與企業之間的金錢流動。',
          ...
          return {
            title: `${titleHeadings}：翻開立委（12年份）的金主投資名冊`,
            description: '誰一直是立委的大金主？原來企業捐獻的政治獻金也會藍綠兩邊押寶？READr 藉由鄉民的幫助擴大了立委政治獻金資料庫，來看看 2008 年至 2020 年 4 次立委選舉中候選人與企業之間的金錢流動。',
            metaUrl: 'political-contribution',
            metaImage: 'political-contribution/ogimgs/ogimage_v2.jpg',
            customScript: structuredDataScript
