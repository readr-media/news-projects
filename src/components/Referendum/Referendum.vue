<template>
  <div class="referendum">
    ...
    <div class="referendum__credit">
      <div class="mirrormedia">
        <img src="/proj-assets/referendum/icons/logo-mm.png" />
      </div>
...
const fetchRefAppDetail = (store, targ_ref) =>
  store.dispatch('Referendum/FETCH_APP_DETAIL', { targ_ref })
export default {
  name: 'Referendum',
  components: {
    Conclusion,
  ...
  mounted() {
    ...
    if (this.isDesktop) {
      Promise.all(
        map(new Array(10), (o, i) =>
          fetchRefAppDetail(this.$store, `no${i + 7}`)
        )
      ).then(() => {
        debug('GOT ALL DATA')
    ...
    window.addEventListener('scroll', () => {
      ...
      if (!this.isGaSent && content_top_y < current_top_y + device_height) {
        window.ga('send', 'event', 'projects', 'scroll', `end`, {
