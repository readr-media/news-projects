...
export function currEnv() {
  if (process.env.VUE_ENV === 'client') {
    if (
      location.host.indexOf(MM_DOMAIN) === 0 ||
      location.host.indexOf(`projects.${MM_DOMAIN}`) === 0 ||
      location.host.indexOf(READR_DOMAIN_PROD) === 0
    ) {
      return 'prod'
    } else {
      return 'dev'
...
export function getReportUrl(slug) {
  return OLD_PROJECTS_SLUGS.includes(slug)
    ? `https://${MM_DOMAIN}/projects/${slug}`
    : `https://${READR_DOMAIN_PROD}/project/${slug}`
}
...
export function isScrollBarReachBottom(ratio = 0, errorMargin = 10) {
  ...
  return (
    getDocHeight() <=
    getScrollXY()[1] + window.innerHeight + vh * ratio + errorMargin
  )
}

export function rafWithDebounce(func) {
  let isTicking = false
