...
import { PROJECTS_GAID_BELONGS_MM } from '../constants'
...
const serverMetaInfoMixin = {
  created () {
    ...
    if (metaInfo) {
      ...
      const favicon = PROJECTS_GAID_BELONGS_MM.includes(project) ? MM_FAVICON_URL : READR_FAVICON_URL
      ...
      const fbPageUrl = PROJECTS_GAID_BELONGS_MM.includes(project) ? MM_FB_PAGE_URL : READR_FB_PAGE_URL

      this.$ssrContext.title = title
      this.$ssrContext.description = description
      this.$ssrContext.locale = locale
      this.$ssrContext.metaUrl = PROJECTS_GAID_BELONGS_MM.includes(project) ? MM_SITE_URL + metaUrl : READR_SITE_URL + metaUrl
      this.$ssrContext.metaImage = PROJECTS_GAID_BELONGS_MM.includes(project) ? MM_SITE_ASSETS_URL + metaImage : READR_SITE_ASSETS_URL + metaImage
...
const clientMetaInfoMixin = {
  mounted () {
    ...
    if (metaInfo) {
      ...
      document.head.querySelector(`meta[property='og:url']`).content = PROJECTS_GAID_BELONGS_MM.includes(project) ? MM_SITE_URL + metaUrl : READR_SITE_URL + metaUrl
      document.head.querySelector(`meta[property='og:image']`).content = PROJECTS_GAID_BELONGS_MM.includes(project) ? MM_SITE_ASSETS_URL + metaImage : READR_SITE_ASSETS_URL + metaImage
